...
export interface SocialLink {
  id: string;
  platform: 'instagram' | 'telegram' | 'youtube' | 'twitter' | 'facebook' | 'discord' | 'website';
  url: string;
}
...
export interface AppState {
  ...
  analytics: number[]; 
  wishlist: string[]; 
  socialLinks: SocialLink[]; // New field for managing social media
  adminPin?: string; // Kept PIN for session security
}
